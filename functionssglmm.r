...
Q.b.poisson <- function(b,Xbeta,Y,det.Sigma,inv.Sigma){
  ...
  dens = sum(dpois(Y,lambda = exp(eta),log=TRUE)) +
         gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
  return(dens)
}
...
Q.b.binomial <- function(b,Xbeta,Y,det.Sigma,inv.Sigma,ntrial){
  eta = Xbeta + b
  p <- inv.logit(eta)
  dens = sum(dbinom(Y,size = ntrial, prob = p,log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
  return(dens)
}
...
Q.b.negbin <- function(b,Xbeta,prec,Y,det.Sigma,inv.Sigma){
  eta = Xbeta + b
  dens = sum(dnbinom(Y, size=prec, mu = exp(eta), log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
...
Q.b.gamma <- function(b,Xbeta,prec,Y,det.Sigma,inv.Sigma){
  eta = Xbeta + b
  mu = exp(eta)
  dens = sum(dgamma(Y, shape=prec, scale = mu/prec,log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
  return(as.numeric(dens))
}
...
Q.b.beta <- function(b,Xbeta,prec,Y,det.Sigma,inv.Sigma){
  eta = Xbeta + b
  mu = inv.logit(eta)
  dens = sum(dbeta(Y, mu*prec, (1-mu)*prec, log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
  return(as.numeric(dens))}
...
glgm2 <- function(formula, cov.model, kappa, inits, data, coords, nugget, family, ntrial=1, offset=1, method.optim, method.integrate, predict=TRUE){
  formula <- as.formula(formula)
  mf <- model.frame(formula,data=data)
  Y <- model.response(mf)
  X <- model.matrix(formula, data=data)
  ...
  if (nugget == T) {
      switch(family,
             "negative.binomial" = names <- c(names,"logtau2", "logprec"),
             "gamma" = names <- c(names, "logtau2", "logprec"),
             "beta" = names <- c(names, "logtau2", "logprec"),
             "poisson" = names <- c(names, "logtau2"),
             "binomial" = names <- c(names, "logtau2")
             )
  }
  if (nugget == F) {
      switch(family,
             "negative.binomial" = names <- c(names, "logprec"),
             "gamma" = names <- c(names, "logprec"),
             "beta" = names <- c(names, "logprec")
             )
  }
...
start.values.sglmm <- function(formula, data, coords, nugget, family, ntrial=1, offset=1){
...
## Minhas funções para integrando de Q:
Q.b <- function(b, Xbeta, Y, det.Sigma, inv.Sigma, family, prec = NULL, ntrial = NULL){
  eta <- Xbeta + b
  dens <- switch (family,
                  "poisson" = sum(dpois(Y,lambda = exp(eta),log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma),
                  "binomial" = sum(dbinom(Y,size = ntrial, prob = (1/(1 + exp(- eta))), log=TRUE)) + gauss.mult(b, det.Sigma = det.Sigma, inv.Sigma = inv.Sigma),
                  "negative.binomial" = sum(dnbinom(Y, size=prec, mu = exp(eta), log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma),
                  "gamma" = sum(dgamma(Y, shape = prec, scale = exp(eta)/prec, log=TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma),
                  "beta" = sum(dbeta(Y, (1/(1 + exp(- eta)))*prec, (1-(1/(1 + exp(- eta))))*prec, log = TRUE)) + gauss.mult(b,det.Sigma = det.Sigma, inv.Sigma = inv.Sigma)
  )
  return(as.numeric(dens))
}

Q.b.grad <- function(b, Xbeta, Y, det.Sigma, inv.Sigma, family, prec = NULL, ntrial = NULL){
  grad <- switch(family,
                 "poisson" = {
                 ...
                 "negative.binomial" = {
                   ...
                   et2 <- exp(b)
                   et12 <- et1 * et2
                   t((Y - (prec + Y) * (et12 * (et12 + prec)^-1))) - t(b) %*% inv.Sigma
                 },
                 "gamma" = {
                   p1 <- -prec + prec * (exp(-Xbeta - b)) * Y
                   t(p1) - t(b) %*% inv.Sigma
...
Q.b.hess <- function(b, Xbeta, Y, det.Sigma, inv.Sigma, family, prec = NULL, ntrial = NULL) {
  Hess <- switch(family,
                 "poisson" = {
                   eta <- exp(Xbeta + b)
                   diag(inv.Sigma) <- eta + diag(inv.Sigma)
...
loglik.sglmm <- function(par, Y, X, kappa, nugget, mat.dist, cov.model, family,metodo,ntrial=1, offset=NA){
  #print(par)
  I = -sqrt(.Machine$double.xmax)
  n <- length(Y)
  n.beta <- dim(X)[2]
  ...
  if (!is.null(kappa)) {kappa=exp(as.numeric(kappa))}
  #if(kappa != "NULL"){kappa=exp(as.numeric(kappa))}
  # Montando a matriz de covariancia
  Sigma <- as.matrix(forceSymmetric(monta.sigma(cov.pars = c(sigma,phi),
  ...
  if(class(chol.Sigma)[1] != "try-error"){
    if(class(inv.Sigma)[1] != "try-error"){
      #### Avaliando a verossimilhança
      ## Caso Poisson
      I <- laplace(Q.b, gr = Q.b.grad, hess = Q.b.hess, metodo = metodo, otimizador="BFGS",
                   n.dim = n, Xbeta = Xbeta, Y = Y, det.Sigma = det.Sigma, inv.Sigma = inv.Sigma,
                   family = family)
    }
  }
...
sglmm_teste <- function(formula, cov.model, kappa, inits, data, coords, nugget, family, ntrial = 1, offset = 1, method.optim, method.integrate, predict = TRUE){
    formula <- as.formula(formula)
    mf <- model.frame(formula,data = data)
    Y <- model.response(mf)
    X <- model.matrix(formula, data = data)
    ...
    if (is.null(inits)) {
        inits <- start.values.sglmm(formula, data, coords, nugget, family, ntrial, offset = offset)
    }
    names(inits) <- names
